...
//重置下次测试的读取计数
	readCount = 0
//提供提示以加快查找速度。特别是，我们给出了最后一次更新的确切位置
...
//尝试获取中间值
//如果我们在现在-年*3+6*月内寻找一个值，我们应该得到这个值
//因为“有效载荷”是时间戳本身，所以我们可以检查它。
...
//提供提示以加快查找速度。特别是，我们给出了最后一次更新的确切位置
	...
	for i := uint64(0); i <= 994; i++ {
T := uint64(now - 1000 + i) //最近1000秒每秒钟更新一次
...
T := uint64(Year * 5 * i) //从1970年1月1日开始每5年写3次更新，然后沉默
		data := Data{
Payload: T, //我们的“有效载荷”将是时间戳本身。
			Time:    T,
		}
...
//CookGetNextletvelTests用于生成确定性
//testGetNextLevel的一组事例，从而“冻结”其当前行为
